...
class Connect4:
    ...
    def drop_token_fixed(self, token, n_column):
        n_row = 0
        while n_row + 1 < self.height and self.board[n_row + 1][n_column] == '_':
            n_row += 1
        self.board[n_row][n_column] = token

    def center_column(self, offset=0):
        return [self.board[n_row][self.width//2 + offset] for n_row in range(self.height)]
    ...
    def heuristic(self, token):
        value = 0
        for four in self.iter_fours():
            if four.count(token) + four.count('_') == 4:
                value += four.count(token)

        value += self.center_column().count(token) * 2

        return value

    @staticmethod
    def negate_token(token):
