...
class AlfaBetaAgent:
    ...
    def decide(self, game=connect4.Connect4()):  # returns int - # of column
        if game.who_moves != self.my_token:
            raise AgentException('not my round')

        move = self.minmax(game=game, depth=self.depth, alfa=-math.inf, beta=math.inf, maximizing_player=True)
        if move is None:
            return random.choice(game.possible_drops())

        return move[0]

    def minmax(self, game, depth, alfa, beta, maximizing_player):
        ...
        if maximizing_player:
            ...
            for col in valid_moves:
                ...
                new_score = self.minmax(game, depth - 1, alfa, beta, False)[1]
                game.board = b_copy
                if new_score > value:
                    value = new_score
                    column = col
                alfa = max(alfa, new_score)
                if alfa >= beta:
                    break
            return column, value
            ...
            for col in valid_moves:
                ...
                new_score = self.minmax(game, depth - 1, alfa, beta, True)[1]
                game.board = b_copy
                if new_score < value:
                    value = new_score
                    column = col
